...
const EmailSecret = require("../models/emailSecret");
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const text = "Welcome to the Project Manager!";
    const html = `Thank you for registering!\nTo finish registration just click this link href = 'http://localhost:5500/api/users/confirm/${emailSecretCode.toHexString()}'`;
    await sendEmail.sendEmail(email, subject, text, html);
    res.status(200).json({
      message:
  ...
  if (user) {
    ...
    try {
      const mailOptions = {
        ...
        html: `<h1>Thank you for registering!</h1></br><p>This is your one time code: ${newSecretCode}\n`,
      };
